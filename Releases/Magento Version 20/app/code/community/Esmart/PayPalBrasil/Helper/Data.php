...
class Esmart_PayPalBrasil_Helper_Data extends Mage_Core_Helper_Data
{
     ...
     * @return string
     */
    public function getCpfCnpjOrTaxvat(Mage_Customer_Model_Customer $customer)
    {
        $cpfData = $customer->getData(Mage::getStoreConfig('payment/paypal_plus/cpf'));

        if (!empty($cpfData)) {
            return $cpfData;
        }

        $cnpjData = $customer->getData(Mage::getStoreConfig('payment/paypal_plus/cnpj'));
     ...
     * @return void
     */
    public function logException($fileOrigin, $class, $method, $line, $file, $exception)
    {
        $data = array();
