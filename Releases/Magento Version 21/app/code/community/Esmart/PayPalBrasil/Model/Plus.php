...
class Esmart_PayPalBrasil_Model_Plus extends Mage_Payment_Model_Method_Abstract
{
    ...
    public function getApiContext()
    {
        ...
        $apiContext->setConfig(array('mode' => $this->getMode(),));
    ...
    protected function createDetails(Mage_Sales_Model_Quote $quote)
    {
        ...
        $details->setSubtotal($totals['subtotal']->getValue());
    ...
    public function getCustomerInformation($quote = null)
    {
        ...
        if ($quote->getCustomerId() == 0) {
            ...
            $customer->setFirstname($quote->getData('customer_'.Mage::getStoreConfig('payment/paypal_plus/guest_firstname')))
                     ->setLastname($quote->getData('customer_'.Mage::getStoreConfig('payment/paypal_plus/guest_lastname')))
                     ->setEmail($quote->getData('customer_'.Mage::getStoreConfig('payment/paypal_plus/guest_email')));

            $cpfField  = Mage::getStoreConfig('payment/paypal_plus/guest_cpf');
            $customer->setData(Mage::getStoreConfig('payment/paypal_plus/cpf'), $quote->getData("customer_{$cpfField}"));

            $cnpjField = Mage::getStoreConfig('payment/paypal_plus/guest_cnpj');
            $customer->setData(Mage::getStoreConfig('payment/paypal_plus/cnpj'), $quote->getData("customer_{$cnpjField}"));
        }
        ...
        $originPhone = ($isGuest ? Mage::getStoreConfig('payment/paypal_plus/guest_phone')
                                 : Mage::getStoreConfig('payment/paypal_plus/phone'));

        $return = array(
            'payerFirstName' => $customer->getData(Mage::getStoreConfig('payment/paypal_plus/firstname')),
            'payerLastName'  => $customer->getData(Mage::getStoreConfig('payment/paypal_plus/lastname')),
            'payerEmail'     => $quote->getCustomerEmail(),
            'payerTaxIdType' => $helper->checkIsCpfOrCnpj($helper->getCpfCnpjOrTaxvat($customer)),
            'payerTaxId'     => $helper->getCpfCnpjOrTaxvat($customer),
            'payerPhone'     => $addressBilling->getData($originPhone),
    ...
    public function createShippingAddress($quote = null)
    {
        ...
        $firstname = $addressShipping->getData(Mage::getStoreConfig('payment/paypal_plus/recipient_firstname'));
        $lastname = $addressShipping->getData(Mage::getStoreConfig('payment/paypal_plus/recipient_lastname'));

        $line1 = $addressShipping->getData(Mage::getStoreConfig('payment/paypal_plus/address_line_1'));
        if (is_numeric(Mage::getStoreConfig('payment/paypal_plus/address_line_1'))) {
            $line1 = $addressShipping->getStreet(Mage::getStoreConfig('payment/paypal_plus/address_line_1'));
        }

        $line2 = $addressShipping->getData(Mage::getStoreConfig('payment/paypal_plus/address_line_2'));
        if (is_numeric(Mage::getStoreConfig('payment/paypal_plus/address_line_2'))) {
            $line2 = $addressShipping->getStreet(Mage::getStoreConfig('payment/paypal_plus/address_line_2'));
        }

        $shipping->setRecipientName("{$firstname} {$lastname}")
            ->setLine1($line1)
            ->setLine2($line2)
            ->setCity($addressShipping->getData(Mage::getStoreConfig('payment/paypal_plus/city')))
            ->setCountryCode($addressShipping->getData(Mage::getStoreConfig('payment/paypal_plus/country_code')))
            ->setPostalCode($addressShipping->getData(Mage::getStoreConfig('payment/paypal_plus/postal_code')))
            ->setState($addressShipping->getData(Mage::getStoreConfig('payment/paypal_plus/state')));
